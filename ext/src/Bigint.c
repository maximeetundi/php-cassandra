...
static HashTable *
php_driver_bigint_gc(zend_object *object, zval **table, int *n)
{
  *table = NULL;
  *n = 0;
...
{
  php_driver_numeric *self =
      (php_driver_numeric *) zend_object_alloc(sizeof(php_driver_numeric), ce);
  ...
  self->std.handlers = &php_driver_bigint_handlers.std;

  return &self->std;
}

void php_driver_define_Bigint(void)
{
  zend_class_entry ce;
  ...
  php_driver_bigint_handlers.std.get_properties  = (zend_object_get_properties_t)php_driver_bigint_properties;
#if PHP_VERSION_ID >= 50400
  php_driver_bigint_handlers.std.get_gc          = (zend_object_get_gc_t)php_driver_bigint_gc;
#endif
