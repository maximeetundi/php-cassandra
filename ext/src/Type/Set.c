...
ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_value_type, 0, 0, PHP_DRIVER_NAMESPACE "\\Type", 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_create_set, 0, 0, PHP_DRIVER_NAMESPACE "\\Set", 0)
...
#if PHP_VERSION_ID >= 80100
  PHP_ME(TypeSet, __construct, arginfo_ctor_none,  ZEND_ACC_PRIVATE)
  PHP_ME(TypeSet, name,        arginfo_string,     ZEND_ACC_PUBLIC)
  PHP_ME(TypeSet, valueType,   arginfo_value_type, ZEND_ACC_PUBLIC)
  PHP_ME(TypeSet, __toString,  arginfo_string,     ZEND_ACC_PUBLIC)
  PHP_ME(TypeSet, create,      arginfo_create_set, ZEND_ACC_PUBLIC)
#else
...
static HashTable *
php_driver_type_set_gc(zend_object *object, zval **table, int *n)
{
  *table = NULL;
  *n = 0;
...
#if PHP_VERSION_ID >= 80000
static HashTable *
php_driver_type_set_properties(zend_object *object)
{
  zval obj_zval;
  ...
  zend_hash_str_update(props,
                       ...
                       &self->data.set.value_type);
  Z_TRY_ADDREF_P(&self->data.set.value_type);
