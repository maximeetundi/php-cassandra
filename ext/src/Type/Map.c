...
ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_key_value_type, 0, 0, PHP_DRIVER_NAMESPACE "\\Type", 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_create_map, 0, 0, PHP_DRIVER_NAMESPACE "\\Map", 0)
...
static zend_function_entry php_driver_type_map_methods[] = {
  ...
  PHP_ME(TypeMap, __toString,  arginfo_string,         ZEND_ACC_PUBLIC)
  PHP_ME(TypeMap, create,      arginfo_create_map,     ZEND_ACC_PUBLIC)
#else
...
static HashTable *
php_driver_type_map_gc(zend_object *object, zval **table, int *n)
{
  *table = NULL;
  *n = 0;
...
#if PHP_VERSION_ID >= 80000
static HashTable *
php_driver_type_map_properties(zend_object *object)
{
  zval obj_zval;
  ...
  zend_hash_str_update(props,
                       "keyType", sizeof("keyType") - 1,
                       &self->data.map.key_type);
  Z_TRY_ADDREF_P(&self->data.map.key_type);

  zend_hash_str_update(props,
                       "valueType", sizeof("valueType") - 1,
                       &self->data.map.value_type);
  Z_TRY_ADDREF_P(&self->data.map.value_type);
